...
if (params$out_name == "none") {
  out_name <- format(Sys.time(), "%Y-%m-%d_%H.%M.%S")
} else {
  out_name <- params$out_name
}
dir_create(path = paste0("Output/", out_name))
...
if (params$new_models == FALSE) {
  model_path <- choose_dir()
}
...
fs::dir_copy(path = "Models",
             new_path = paste0("Output/", out_name, "/Models"))
save.image(file = paste0("./Output/", out_name, "/global.RData"))
```
