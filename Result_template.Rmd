...
#   as.data.frame() %>%
#   rownames_to_column() %>% 
#   `colnames<-`(value = c("Parameter", "Value")) %>% 
#   my_table(caption = "<center><strong>LGBM parameters</center></strong>")

# SHAP plots
lgbm_shap_plot %>%
  walk(print)
# Features tables
feature_data <- imported_data[[env_name]] %>%
  as.data.frame() %>%
  select(c(
    gsub(
      "\\.",
      "/",
      lgbm_shap_plot$importance_plot$data$feature
    ),
    predict_label
  ))
...
#   rownames_to_column() %>% 
#   `colnames<-`(value = c("Parameter", "Value")) %>% 
#   my_table(caption = "<center><strong>XGBoost parameters</center></strong>")
...
feature_data <- imported_data[[env_name]] %>%
  ...
  select(c(
    gsub(
      "\\.",
      "/",
      xgboost_shap_plot$importance_plot$data$feature
    ),
    predict_label
  ))
